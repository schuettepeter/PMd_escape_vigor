function[aligned, transforms] = hyperalign_pls(varargin)
...
%alignment
%
% NOTE: The Statistics Toolbox is needed to usethis function.
%
%Usage:
...
template = zeros(size(varargin{1}'));
for s = 1:length(varargin)        
        template = template + varargin{s}';
end
...
%         [~, next] = procrustes((template./(s))', varargin{s}');
%         [~, next] = procrustes((template./(s - 1))', varargin{s}');
        [~,~,~,~,b] = plsregress(varargin{s}',(template./(s - 1))',100);
        next = [ones( varargin{s}',1) varargin{s}']*b;      
...
for s = 1:length(varargin)
    [~,~,~,~,b] = plsregress(varargin{s}',template',100);
    next = [ones( varargin{s}',1) varargin{s}']*b;   
...
%     [~, next, transforms{s}] = procrustes(template2', varargin{s}');
    [~,~,~,~,transforms{s}] = plsregress(varargin{s}',template2',100);
    next = [ones( varargin{s}',1) varargin{s}']*transforms{s};  
